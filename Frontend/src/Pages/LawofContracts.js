    ...
    const fetchBlogs = async () => {
      try {
        ...
        const filteredBlogs = response.data.filter(blog =>
          ["The Law of Contracts"].includes(blog.category)
        );
    ...
    <div>
      ...
      <NavBottom title="Constitution Of India" />
        ...
        {blogData.length > 0 ? (
          ...
          <div className="w-full text-center text-gray-500">No blogs found related to The Law of Contracts.</div>
        )}
      </div>
      <Footer />
    </div>
