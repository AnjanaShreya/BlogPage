...
const Civilprocedure = () => {
    ...
    const fetchBlogs = async () => {
      try {
        ...
        const filteredBlogs = response.data.filter(blog =>
          ["Civil Procedure 1908"].includes(blog.category)
        );
  ...
  return (
    <div>
      ...
      <NavBottom title="Constitution Of India" />
			{/* content goes here */}
        ...
        {blogData.length > 0 ? (
          ...
          <div className="w-full text-center text-gray-500">No blogs found related to Civil Procedure 1908.</div>
        )}
      </div>
      <Footer />
    </div>
